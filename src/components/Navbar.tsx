...
function Navbar(props: { className?: string; margin?: boolean }) {
  ...
  useEffect(() => {
    ...
    lenis.on("scroll", (e) => {
      ...
      if (e.direction == 1) setHasScrolledUp(false);
      if (e.direction == -1) setHasScrolledUp(true);
    });
    return () => {
      //cleanup how?? TODO
      // lenis.destroy();
  ...
  return (
    <>
      ...
      <header
        className={`hidden bg-white/20 bg-opacity-20 backdrop-blur-lg rounded-lg drop-shadow-lg md:block fixed top-0 lg:top-0 left-0 w-full z-30 ${
          hasScrolledUp ? "" : "-translate-y-full"
        } ${
          isOnTop
            ? "border-text/0"
            : "bg-white/50 bg-opacity-20 backdrop-blur-lg rounded-lg drop-shadow-lg"
        } border-b-2 duration-300 px-16`}
      >
        <div
